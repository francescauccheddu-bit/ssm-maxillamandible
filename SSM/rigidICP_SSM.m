...
% Suppress iteration error display
errortemp(1,:)=0;
index=2;
[errortemp(index,:),Reallignedsourcetemp]=ICPmanu_allign2primo(Prealligned_target,Prealligned_source);

while (abs(errortemp(index-1,:)-errortemp(index,:)))>0.000001
[errortemp(index+1,:),Reallignedsourcetemp]=ICPmanu_allign2primo(Prealligned_target,Reallignedsourcetemp);
index=index+1;
d=errortemp(index,:);
